...
def validate_orders_bytes(data: bytes) -> ValidatedOrders:
    ...
    try:
        ...
        raise PreservationError("Orders file is not valid UTF-8 CSV.") from exc
    if text.lstrip().lower().startswith(("<!doctype html", "<html")):
        raise PreservationError("Downloaded content is HTML, not CSV.")
    ...
    rows = list(reader)
    if not rows:
        raise PreservationError("Orders CSV contains a header but no rows.")
    ...
    if len(order_dates) != 1:
        raise PreservationError("Orders CSV must contain exactly one order_date.")
    ...
    return ValidatedOrders(
        ...
        product_id_column=(
            "new_product_id" if fieldnames == NEW_PRODUCT_ID_COLUMNS else "product_id"
        ),
    )
